...
class Hybrid(Auto):
    def __init__(self, model, engine_type, acu_cap):
        super().__init__(model, engine_type) # wywolanie konstruktora Auto
        self.acu_cap = acu_cap
    def __str__(self):
        return super().__str__() + "\naku: %13s" % (self.acu_cap)
...
class AutoComparator:
    def compareAutos(self, a1, a2):
        ...
        if(a1.__class__.__name__ == "Hybrid"):
            print("%15s | %15s" % (a1.acu_cap, "B/D"))
        if(a2.__class__.__name__ == "Hybrid"):
